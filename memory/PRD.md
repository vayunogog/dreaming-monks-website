...
- 2026-08-16: Looping video added to What We Offer — first card (Outdoor LED Screens) placeholder replaced with the same seamless VideoPlaylist (OFFER_VIDEOS = sites-1/2); other two cards keep marked placeholders. Verified: playlist state machine + both footage frames visible in card. Hero photos: confirmed the uploaded photo-1 file itself is only 547x365 (29KB — no larger original exists in the asset pipeline, verified via HEAD/variant requests); hero-2 (Corevo) re-saved at native 1000x700 without upscaling. Photo 1 stays as interim upscale until user sends full-res original.
- 2026-08-16: Hero slideshow replaced with 5 user photos (Tanishq elevator, Mercedes lobby, Nike elevator, Prateek edifice real photo, LUMINANCE real photo) — downloaded at native 1500-2000px, optimized to progressive JPEGs (~270-325KB each, /public/photos/hero-1..5.jpg). Artifact order came back reversed vs message and file↔content mapping was verified in-browser per slide before final ordering: HERO_PHOTOS = [hero-5, hero-3, hero-4, hero-2, hero-1] matching user's message order. Same 5s/1s crossfade, cycles all 5 (25s loop).
